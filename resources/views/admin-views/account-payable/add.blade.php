        ...
        <form action="{{ route('admin.account.store-payable') }}" method="post" id="openingBalanceForm" class="p-4" novalidate>
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-6">
                    ...
                    {{-- إجماليات الحساب --}}
                    <div class="mt-3">
                        <div class="hint mb-2">{{ \App\CPU\translate('إجماليات الحساب الحالية') }}</div>
                        <div class="totals-wrap">
                            <div class="totals-pill" id="pillTotalIn">
                                <span class="badge-soft">{{ \App\CPU\translate('إجمالي مدين (total_in)') }}</span>
                                <span id="totalInValue">0.00</span>
                                <button type="button" class="btn btn-outline-success btn-xs" id="btnCopyToDebit">
                                    {{ \App\CPU\translate('نسخ إلى المدين') }}
                                </button>
                            </div>
                            <div class="totals-pill" id="pillTotalOut">
                                <span class="badge-soft">{{ \App\CPU\translate('إجمالي دائن (total_out)') }}</span>
                                <span id="totalOutValue">0.00</span>
                                <button type="button" class="btn btn-outline-primary btn-xs" id="btnCopyToCredit">
                                    {{ \App\CPU\translate('نسخ إلى الدائن') }}
                                </button>
                            </div>
                            <div class="totals-pill">
                                <span class="badge-soft">{{ \App\CPU\translate('الصافي (مدين - دائن)') }}</span>
                                <span id="netValue">0.00</span>
                                <button type="button" class="btn btn-outline-secondary btn-xs" id="btnNetToSide">
                                    {{ \App\CPU\translate('وضع الصافي تلقائيًا') }}
                                </button>
                            </div>
                        </div>
                    </div>
