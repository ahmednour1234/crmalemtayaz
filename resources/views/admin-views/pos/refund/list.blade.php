...
<div class="content container-fluid">
  ...
  <div class="mb-3">
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb bg-white px-3 py-2 rounded shadow-sm mb-0">
        ...
        <li class="breadcrumb-item active">{{ \App\CPU\translate('فواتير المرتجعات') }}</li>
      </ol>
    </nav>
  </div>
  ...
  <div class="filter-card mb-3">
    <div class="filter-head">
      <h4>{{ \App\CPU\translate('بحث وفلاتر المرتجعات') }}</h4>
      @if($hasFilters)
  ...
  <div class="card">
    <div class="card-header d-flex align-items-center justify-content-between">
      ...
      @if($hasFilters)
        <small class="text-muted">{{ \App\CPU\translate('عدد السجلات الظاهرة') }}: {{ $refunds->total() ?? $refunds->count() }}</small>
      @endif
    </div>
      ...
      <div class="card-body" id="product-table">
        ...
        <div class="table-responsive">
          <table id="invoicesTable" class="table ">
            ...
            <tbody id="set-rows">
            @foreach($refunds as $key=>$order)
              <tr class="status-{{$order['order_status']}} class-all">
                <td class="none">{{$key+$refunds->firstItem()}}</td>
