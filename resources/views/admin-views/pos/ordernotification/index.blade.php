            ...
            <div class="row align-items-center mb-3">
                <div class="col-sm">
                    <h1 class="page-header-title text-capitalize">{{\App\CPU\translate('pos')}} {{\App\CPU\translate('orders')}}
                                      <span class="badge badge-soft-dark ml-2">{{ $notifications->count() }}</span>
</h1>
                </div>
            </div>
        </div>
        ...
        <!-- Card -->
            ...
            <div class="row align-items-center">
                <div class="col-lg-4 mb-3 mb-lg-0">
                    <input type="text" name="search" class="form-control" placeholder="Search by seller or customer name">
                </div>
                <div class="col-lg-2">
        ...
        <div class="card">
            ...
            <div class="table-responsive ">
                ...
                >
                    ...
                    <tbody id="set-rows">
                    @foreach($notifications as $key=>$order)
                        <tr class="status-{{$order['order_status']}} class-all">
                            <td class="">
                                {{$key+$notifications->firstItem()}}
                            </td>
                            ...
                            <td>{{ $order['type'] == 4 ? 'Order' : 'Refund' }}</td>
                            <td>{{ $order->extra_discount ? number_format($order->extra_discount, 2) : 0  }}</td>
                            <td>{{ $order->coupon_discount_amount ? number_format($order->coupon_discount_amount, 2) : 0 }}</td>
                            <td>{{ number_format($order->order_amount + $order->total_tax - $order->extra_discount - $order->coupon_discount_amount, 2) }}</td>
                            <td>
                               <a href="{{ route('admin.ordernotification.show', ['order_id' => $order->id]) }}">
...
@push('script_2')
    <script>
        ...
        function print_invoice(order_id) {
            $.get({
                url: '{{url('/')}}/admin/ordernotification/' + order_id,
                dataType: 'json',
                beforeSend: function () {
                    $('#loading').show();
                },
