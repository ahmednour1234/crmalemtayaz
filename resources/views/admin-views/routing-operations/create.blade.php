{{-- resources/views/admin-views/routing-operations/create.blade.php --}}
@extends('layouts.admin.app')

@section('title', 'إضافة خطوة تشغيل')
...
<div class="content container-fluid" dir="rtl">
    <div class="card form-card">
        <div class="card-header">إضافة خطوة تشغيل</div>
        <div class="card-body">
            <form action="{{ route('admin.routing-operations.store') }}" method="POST">
                @csrf
                <div class="row gx-4 gy-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="work_center_id" class="form-select select2-single" required>
                            <option value="" disabled selected>اختر مركز العمل</option>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" name="sequence" class="form-control" value="{{ old('sequence') }}" min="1" required>
                        @error('sequence')<div class="text-danger mt-1">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">زمن الإعداد (ساعي)</label>
                        <input type="number" step="0.01" name="setup_time" class="form-control" value="{{ old('setup_time') }}" required>
                        @error('setup_time')<div class="text-danger mt-1">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">زمن التشغيل (ساعي)</label>
                        <input type="number" step="0.01" name="run_time" class="form-control" value="{{ old('run_time') }}" required>
