...
@section('title', \App\CPU\translate('تعديل تقييم المقابلة'))
...
<div class="content container-fluid">
  ...
  <div class="mb-3">
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb bg-white px-3 py-2 rounded shadow-sm">
        ...
        <li class="breadcrumb-item active">{{ \App\CPU\translate('تعديل تقييم المقابلة') }}</li>
      </ol>
    </nav>
  </div>
  ...
  <div class="card-soft p-3">
    <div class="page-head mb-2">
      <h1 class="mb-0">{{ \App\CPU\translate('تعديل تقييم المقابلة') }}</h1>
      <div class="btn-bar">
    ...
    <form action="{{ route('admin.interview_evaluations.update', $evaluation->id) }}" method="POST" novalidate>
      @csrf
      @method('PUT')
      ...
      <div class="grid grid-3">
        <div>
          ...
          <input type="text" name="interviewer"
                 ...
                 value="{{ old('interviewer', $evaluation->interviewer) }}"
                 placeholder="{{ \App\CPU\translate('ادخل اسم المُقابِل') }}" required>
        ...
        <div>
          ...
          <input type="date" name="interview_date"
                 ...
                 value="{{ old('interview_date', optional($evaluation->interview_date)->format('Y-m-d')) }}" required>
      ...
      <div class="mt-3">
        ...
        <textarea name="evaluation_notes" rows="4"
                  ...
                  placeholder="{{ \App\CPU\translate('ادخل الملاحظات') }}">{{ old('evaluation_notes', $evaluation->evaluation_notes) }}</textarea>
      ...
      <div class="btn-bar mt-3">
        <button type="submit" class="btn btn-primary">
          <i class="tio-checkmark-circle-outlined"></i> {{ \App\CPU\translate('تحديث') }}
        </button>
