{{-- resources/views/admin-views/bom-components/edit.blade.php --}}
...
<div class="content container-fluid" dir="rtl">
    <div class="card form-card">
        <div class="card-header">تعديل مكون</div>
        <div class="card-body">
            <form action="{{ route('admin.bomcomponents.update', $component->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="bom_id" class="form-select select2-single" required>
                            ...
                            @foreach($boms as $bom)
                                <option value="{{ $bom->id }}" {{ old('bom_id', $component->bom_id) == $bom->id ? 'selected' : '' }}>
                                    {{ $bom->product->product_code }} – {{ $bom->product->name }} ({{ $bom->version }})
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="component_product_id" class="form-select select2-single" required>
                            <option value="" disabled>اختر المكون</option>
                            @foreach($products as $prod)
                                <option value="{{ $prod->id }}" {{ old('component_product_id', $component->component_product_id) == $prod->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" step="0.0001" name="quantity" class="form-control"
                               value="{{ old('quantity', $component->quantity) }}" required>
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="unit" id="unit_main" value="0"
                                {{ old('unit', $component->unit) == 0 ? 'checked' : '' }}>
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="unit" id="unit_sub" value="1"
                                {{ old('unit', $component->unit) == 1 ? 'checked' : '' }}>
...
@endsection
    ...
    <script>
        $(function(){ $('.select2-single').select2({ placeholder: 'اختر', width: '100%' }); });
    </script>
