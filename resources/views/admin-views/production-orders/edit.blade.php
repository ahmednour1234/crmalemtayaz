{{-- resources/views/admin-views/production-orders/edit.blade.php --}}
...
<div class="content container-fluid" dir="rtl">
    <div class="card form-card">
        <div class="card-header">تعديل أمر إنتاج</div>
        <div class="card-body">
            <form action="{{ route('admin.production-orders.update', $order->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row gx-4 gy-4">
                 
                    <div class="col-md-6">
                        <label class="form-label">الوصفة (BOM)</label>
                        <select name="bom_id" class="form-select select2-single" required>
                            <option value="" disabled>اختر الوصفة</option>
                            @foreach($boms as $b)
                                <option value="{{ $b->id }}"
                                    {{ old('bom_id', $order->bom_id) == $b->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="routing_id" class="form-select select2-single" required>
                            ...
                            @foreach($routings as $r)
                                <option value="{{ $r->id }}"
                                    {{ old('routing_id', $order->routing_id) == $r->id ? 'selected' : '' }}>
                                    {{ $r->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" name="quantity" class="form-control" step="0.01"
                               value="{{ old('quantity', $order->quantity) }}" placeholder="مثلاً: 100.00" required>
...
@if($order->status==='draft')
                    <div class="col-md-4">
                        ...
                        <select name="status" class="form-select" required>
                            <option value="draft" {{ old('status', $order->status)=='draft'? 'selected':'' }}>مسودة</option>
                            <option value="planned" {{ old('status', $order->status)=='planned'? 'selected':'' }}>مخطط</option>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" name="start_date" class="form-control"
                               value="{{ old('start_date', optional($order->start_date)->format('Y-m-d')) }}">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" name="end_date" class="form-control"
                               value="{{ old('end_date', optional($order->end_date)->format('Y-m-d')) }}">
                        @error('end_date')<div class="text-danger mt-1">{{ $message }}</div>@enderror
                    </div>

                    <div class="col-12 text-center mt-4">
                        <button type="submit" class="btn btn-submit">تحديث الأمر</button>
