            ...
            <li class="breadcrumb-item">
               ...
               <li class="breadcrumb-item">
                <a href="#" class="text-primary">
                    {{ \App\CPU\translate('تقرير المركز ') }}{{$costCenter->name}}
                </a>
            </li>
        </ol>
    </nav>
</div>
        <!-- نموذج لتصفية التقرير بحسب التاريخ -->
...
<div class="row mt-4">
    ...
    <tbody>
        @php
            // دالة تكرارية لعرض شجرة الحسابات مع رقم تسلسلي
            function renderAccountTree($nodes, $level = 0, &$index = 1) {
                $html = '';
                foreach ($nodes as $node) {
                    // حساب المسافة للتداخل
                    $padding = $level * 20;

                    $html .= '<tr data-level="' . $level . '">';
                    $html .= '<td>' . $index++ . '</td>'; // رقم الصف
                    $html .= '<td style="padding-left:' . $padding . 'px;">' . $node['account_name'];
                    if ($level > 0) {
                        $html .= ' (حساب فرعي)';
                    }
                    $html .= '</td>';
                    $html .= '<td>' . number_format($node['debit'], 2) . '</td>';
                    $html .= '</tr>';

                    // عرض الأطفال إذا وجدت
                    if (isset($node['children']) && count($node['children']) > 0) {
                        $html .= renderAccountTree($node['children'], $level + 1, $index);
                    }
                }
                return $html;
            }
            ...
            // بدء الطباعة
            $index = 1;
            echo renderAccountTree($accountsTree, 0, $index);
        @endphp
    </tbody>
    <tfoot>
        <tr>
            <th colspan="2" class="text-end">{{ \App\CPU\translate('الإجمالي') }}</th>
